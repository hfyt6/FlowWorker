...
namespace FlowWorker.Core.Tools.VersionControl;
...
public class VersionControlTool : IToolHandler
{
    ...
    public async Task<ToolResponse> ExecuteAsync(string action, JsonElement parameters)
    {
        return action.ToLowerInvariant() switch
        {
            ...
            "git_push" => await GitPushAsync(parameters),
            "git_pull" => await GitPullAsync(parameters),
    ...
    private Task<ToolResponse> GitPushAsync(JsonElement parameters)
    {
        try
        {
            var repositoryPath = parameters.GetProperty("repository_path").GetString();
            var remote = parameters.TryGetProperty("remote", out var remoteProp) ? remoteProp.GetString() : "origin";
            var branch = parameters.TryGetProperty("branch", out var branchProp) ? branchProp.GetString() : null;
            ...
            var gitHelper = new GitHelper(repositoryPath);
            var result = gitHelper.Push(remote, branch);
        ...
        {
            return Task.FromResult(ToolResponse.Error("EXECUTION_FAILED", $"推送代码失败：{ex.Message}"));
        }
    }

    private Task<ToolResponse> GitPullAsync(JsonElement parameters)
    {
        try
        {
            var repositoryPath = parameters.GetProperty("repository_path").GetString();
            ...
            var gitHelper = new GitHelper(repositoryPath);
            var result = gitHelper.Pull(remote, branch);
        ...
        {
            return Task.FromResult(ToolResponse.Error("EXECUTION_FAILED", $"拉取代码失败：{ex.Message}"));
        }
    }
...
public class GitHelper
{
    ...
    public PushPullResult Push(string remote, string? branch)
    {
        var result = new PushPullResult();
        ...
        try
        {
            ...
            ExecuteGitCommand(args.ToString());
            result.Success = true;
            result.Message = "推送成功";
        }
        catch (Exception ex)
    ...
    public PushPullResult Pull(string remote, string? branch)
    {
        var result = new PushPullResult();
