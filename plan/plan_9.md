...
- [ ] 创建测试项目结构：
  ...
  - FlowWorker.Api.Tests（API 控制器测试）
...
- [ ] 编写 Service 层单元测试：
  ...
  - RoleService 测试（角色管理）
  - PromptTemplateService 测试（提示词模板）
  - 目标覆盖率 > 80%
- [ ] 编写 Repository 层单元测试：
  - 使用 InMemory 数据库
  - 测试所有 CRUD 操作
...
- [ ] docs/PROMPT_ENGINEERING.md（提示词工程指南）
- [ ] CHANGELOG.md（变更日志）
- [ ] LICENSE（许可证）
